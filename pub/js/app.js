...
//Menu en Responsive, abrir o cerrar
  ...
  $(".recordar").on('click',function(){
      ...
      $("fieldset").append("</form><form method='get' action='recup.php' class='recup'><div class='row'><input type='text' class='login' placeholder='Email' name='recordar' /></div><div class='row'><input type='submit' id='recup' value='Enviar' /></div></form>");
    ...
    $(".vuelta").on('click',function(){
      ...
      $("fieldset").append("<div class='row'><input type='text' class='login' name='email' placeholder='Usuario' /></div><div class='row'><input type='password' class='password' name='password' placeholder='Contraseña'/></div><div class='row'><input type='submit' id='log' value='Ingresar' /></div>");
    });
   ...
   if(aux_path[aux_path.length-1] == 'home' || aux_path[aux_path.length-1] == ''){
      var postData = $(this).serialize();
         $.ajax({
           ...
           success:function(output){
              for(i = 0; i < output.length; i++){
                $(".juegos").append("<div class='item-juego' id='"+output[i]['nombre']+"'><a href='../M-master/game'><img class='imagen-juego' src='"+output[i]['fuente']+"' /></a><div class='text-game'><a href='../M-master/game'><div class='juega'></div><div>"+output[i]['nombre']+"</div></br></a></div><a href='../M-master/game'></a></div>");
              }
